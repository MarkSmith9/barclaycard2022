...
### Payment gate variables
note: we should hide these variables from the customer and send them directly to the check out page
- transaction uuid - auto generated by file - seems to accept arbitrary input as long as is unique
- locale - set to en
- transaction type - set to authorisation
- reference number - auto generated in file based on date, accepts arbitrary input so tranaction info could be in there
- currentcy - hidden set to gbp
- generated in file
- access key - hidden, set to the key
- profile_id - hidden, set to the id
